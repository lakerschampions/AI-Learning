'''
The purpose of this code is to demonstrate how to use twython to connection to Twitter's streaming API and download tweets of a certain language.
...
class MyStreamer(twy.TwythonStreamer):
    ...
    def on_success(self, data):
        ...
        fileName = self.fileDirectory + 'Tweets_' + dt.datetime.now().strftime(
            "%Y_%m_%d_%H") + '.txt'  # File name includes date out to hour.
...
def streamConnect(APP_KEY, APP_SECRET, OAUTH_TOKEN, OAUTH_TOKEN_SECRET):
    ...
    stream.statuses.sample(track=['a,an,the,and,but,is,this,that,on,in,up,to'], language=['en'])

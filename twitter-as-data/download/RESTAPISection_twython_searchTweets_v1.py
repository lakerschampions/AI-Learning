...
os.chdir(
    '/Users/chuzhengtian/Desktop/twitter/')
...
nba_tweets2 = connection.search(q='nba',
                                count=size)  # Search queries launched almost simultaneously return different results.
...
nba_tweets5 = connection.search(q='nba -Cavs -Cavaliers -Warriors',
                                count=size)
# Tweets with nba but not Cavs, Cavaliers, or Warriors.  88 at the time of the query.
...
# Geocode
nba_tweets_nyc = connection.search(q='nba', count=size,
                                   geocode='40.7128,-74.0059,5mi')
# Return tweets within a 500 mile radius of New York City.

# Since date. n has to be large or the topic small for this parameter to limit the number of tweets returned.
taxes = connection.search(q='tax', count=size,
                          since_id=1280047623093641216)
# since_id is ID of tweet, not a date.  No functionality for since a specific date.

# Until date
taxes = connection.search(q='tax', count=size, until='2020-07-06')  # Tweets before the until date.  Note that search
# does not return tweets older than 7 days.
...
delay = (window_minutes * 60) / requestsPer15Window  # Calculates how many seconds per request per window.  This
# number will tell the loop how many seconds to sleep.

# Build dataset.  Modify the while loop to collect data in a way consistent with your research needs.  For example,
# perhaps you download tweets every minute, so time.sleep(delay) will be time.sleep(60).  Or perhaps you make a
# simple function that you run every minute as a cron job.
i = 0
...
while i <= 10:  # Run 10 cycles.
    ...
    next_tweets = connection.search(q='nba', count=size, since_id=nba_tweets[total][
        'id'])  # Only get tweets matching the criteria since the last tweet we have
...
# Pause for delay seconds.  Has the function of both not tripping the rate limits and ensuring some tweets will
# occur.  If your search criteria return sufficiently few tweets, the loop stops.
